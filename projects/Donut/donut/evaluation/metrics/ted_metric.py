...
class JSONParseEvaluator(BaseMetric):
    """Calculate n-TED(Normalized Tree Edit Distance) based accuracy and F1
    accuracy score."""
    ...
    def update_cost(node1: Node, node2: Node):
        ...
        If both are leaf node, calculate string edit distance between two
        labels (special token '<leaf>' will be ignored). If one of them is leaf
        node, cost is length of string in leaf node + 1. If neither are leaf
        node, cost is 0 if label1 is same with label2 otherwise 1
        """
        label1 = node1.label
        label2 = node2.label
    ...
    def insert_and_remove_cost(node: Node):
        ...
        If leaf node, cost is length of label name. Otherwise, 1
        """
        label = node.label
        if '<leaf>' in label:
    ...
    def cal_f1(self, preds: List[dict], answers: List[dict]):
        """Calculate global F1 accuracy score (field-level, micro-averaged) by
        counting all true positives, false negatives and false positives."""
        total_tp, total_fn_or_fp = 0, 0
        for pred, answer in zip(preds, answers):
            pred, answer = self.flatten(
                self.normalize_dict(pred)), self.flatten(
    ...
    def construct_tree_from_dict(self,
    ...
    def cal_acc(self, pred: dict, answer: dict):
        """Calculate normalized tree edit distance(nTED) based accuracy. 1)
        Construct tree from dict, 2) Get tree distance with
        insert/remove/update cost, 3) Divide distance with GT tree size (i.e.,
        nTED),
