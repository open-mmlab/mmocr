...
class TestLoadImageFromRecogLMDB(TestCase):
    ...
    def test_init(self):
        with self.assertRaises(ValueError):
            LoadImageFromRecogLMDB(file_client_args=dict(backend='disk'))

    def test_transform(self):
        transform = LoadImageFromRecogLMDB()
    ...
    def test_invalid_key(self):
        ...
        transform = LoadImageFromRecogLMDB(ignore_empty=True)
        results = transform(copy.deepcopy(self.results2))
        self.assertIsNone(results)
    ...
    def test_to_float32(self):
        transform = LoadImageFromRecogLMDB(to_float32=True)
        results = transform(copy.deepcopy(self.results1))
        self.assertIn('img', results)
        self.assertIsInstance(results['img'], np.ndarray)
    ...
    def test_repr(self):
        transform = LoadImageFromRecogLMDB()
        assert repr(transform) == ('LoadImageFromRecogLMDB(ignore_empty=False,'
