...
在 Python 接口中，`MMOCRInferencer` 以字典的形式返回预测结果。字典的键以任务名开头，例如 `det`、`rec` 和 `kie`；字典的值是对应的预测结果。取决于 `MMOCRInferencer` 正在运行的任务，具体返回的结果可能是以下内容的一个子集。
...
除了从返回值中获取预测结果，你还可以通过设置 `out_dir` 和 `save_pred` / `save_vis` 参数以将预测结果及可视化结果导出到文件中。假设 `out_dir` 是 `outputs`，输出的文件将会按如下方式组织：
...
`MMOCRInferencer` 可以通过 CLI 和 Python 接口使用。CLI 和 Python 接口的所有参数都是一样的，你只需要在参数前面加上两个"-"，然后把下划线"\_"替换成连字符"-"即可。(如 `out_dir` 会变成 `--out-dir`)

对于布尔类型的参数，将参数放在命令中就相当于将其指定为 True。
