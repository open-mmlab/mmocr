...
1. Benefiting from the unified designing, you can use the models implemented in other OpenMMLab projects such as MMDet now. We provide an example to demonstrate how to use MMDetection's Mask R-CNN through `MMDetWrapper`. Check our documents for more details. More wrappers will be released in the future.
2. Stronger visualizer and more analysis tools. We provide a series of useful tools which are mostly based on the brand new visualizers. It is more convenient now for the users to explore the models and datasets.
