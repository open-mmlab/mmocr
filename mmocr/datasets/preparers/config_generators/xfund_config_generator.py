...
class XFUNDREConfigGenerator(BaseDatasetConfigGenerator):
    ...
    The main difference with `XFUNDSERConfigGenerator` is:
        - train_anns/val_anns/test_anns default file name:
            f'{ser or re}_{train or test}.json'
        - the value of self.task: 'ser' or 're'
    """
    ...
    def _gen_dataset_config(self) -> str:
        """Same as `XFUNDSERConfigGenerator._gen_dataset_config()`"""
        cfg = ''
        for key_name, ann_dict in self.anns.items():
            cfg += f'\n{key_name} = dict(\n'
            cfg += '    type=\'XFUNDDataset\',\n'
